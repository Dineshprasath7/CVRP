...
def solveMTZ(vertices, edges, capacity_Limit, demand):
    # Define LP Problem
    prob = LpProblem("CVRP", LpMinimize)

    # Variables: Binary for each edge (is edge used), continuous for u_i (positions
    x = LpVariable.dicts('x', [(i,j) for i in vertices for j in vertices if i != j], 0, 1, LpBinary)
    u = LpVariable.dicts('u', vertices, 0, len(vertices) - 1, LpContinuous)

    # Objective: Minimize total distance
    prob += lpSum(edges[(min(i,j), max(i,j))] * x[i, j] for i in vertices for j in vertices if i != j)

    # Constraints
    for i in vertices:
        # Ensure each city is visited exactly once
        prob += lpSum(x[i,j] for j in vertices if i != j) == 1
        prob += lpSum(x[j,i] for j in vertices if i != j) == 1

    # Capacity constraints: subtour elimination (MTZ constraints)
    for i in vertices:
        for j in vertices:
            if i != j:
                prob += u[i] - u[j] + (len(vertices) - 1) * x[i,j] <= len(vertices) - 2
    ...
    # Return the solution and total distance
...
def main():
    ...
    if model == "ACO":
        ...
        for i in range(iterations):
            ...
            print(f"{i}: {int(bestSolution[1])}\tOptimal Value: {optimal_Value}")
        
        # Calculate and display accuracy for ACO
        obtained_distance = int(bestSolution[1])
        print(f"ACO Solution Distance: {obtained_distance}, Optimal Distance: {optimal_Value}")
        return bestSolution

    elif model == "MTZ":
        mtz_solution, total_distance = solveMTZ(vertices, edges, capacity_Limit, demand)
        print(f"MTZ Total Distance: {total_distance}, Optimal Distance: {optimal_Value}")
        return mtz_solution
